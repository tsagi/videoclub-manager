...
void selection1(void) {
  ...
  while ( !feof(fp) ) {
    ...
    if ( key == -1 ) {
      continue; // If this id has already been deleted, then move on
    }
    deleteProcedure(key);
  }
  fclose(fp);
...
void insertProcedure(char *filename) {
  ...
  while ( !feof(fp) ) {
    ...
    while ( isspace(*(line + i)) ){
      i++;   // Ignore any white spaces, and in the first non-
...
int collectid(char *id) {
  ...
  while ( (c = getch()) ) {
    ...
    if ( isspace(c) ) {
      continue;   // Ignore any white spaces
    }
...
int collectids6(void) {
  ...
  while ( (c = getch()) != '\n' ) {
    if ( c == ' ' || c == '\t' ) {
      continue;   // Ignore any spaces or tabs
    }
...
int hashinsert(char *id) {
  ...
  while ( i < ahashsize ) {   // While the cells haven't all been checked
    ...
    // If the ids match
    else if ( strcmp(ahash[j]->id,id) == 0 ) {
      return j;   // Return position on the hash table
    }
    else {
      collisions++;
